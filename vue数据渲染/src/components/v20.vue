<template>
  <div id="c20" class="sales" :style="'margin-top:'+content.modulePadding+'px;margin-bottom:'+content.modulePadding+';'">
    <!--推荐商品-->
    <div>
      <!--倒计时时间-->
      ...
      <div class="salessrc">
        ...
        <span :class="{'spancolor':changestate}" :title="content.act_status">
                <b style="width:40px;display:inline-block;font-weight: 100;margin-top: 5px;">{{content.act_status | str(4)}}</b>
            </span>
      </div>
      <!--商品详情-->
      <div class="salesdetail" @click="herfs(content.goodslist[0].link)">
        <!--商品标题-->
        ...
        <p class="salesprice" style="margin:7px 0;">
          ...
          <!--现价-->
          <span v-show="!nowstate" :title="content.goodslist[0].price">&yen;{{content.goodslist[0].price}}</span>
          <span v-show="nowstate" style="color:#FF0000;font-weight:600" :title="content.goodslist[0].original_price">&yen;{{content.goodslist[0].original_price}}</span>
          <!--原价-->
          <del :class="{'del':delstate}" :title="content.goodslist[0].original_price">&yen;{{content.goodslist[0].original_price}} 元</del>
        </p>
      </div>
    </div>
  </div>
    ...
    methods:{
      timer(){
        ...
        let time = setInterval(function(){
          let endTime = new Date('2019/01/31 09:49:00');
