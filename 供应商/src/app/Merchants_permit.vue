<template>
  <div class='contract buss'>
    <div style="text-align: center;position:relative;margin-top:1em;">
      <p class='imgsrc'><img @click="addFile"  :src="downCard" style="width:90%;margin:1rem auto;height:10em;" alt=""/></p>
    </div>
    ...
    <flexbox  class="x-btn">
      <flexbox-item style='position:relative'>
        <x-button  type="warn" @click="addFile" style="background-color: #fff;color:black;font-size: 16px;">重新上传</x-button>
      </flexbox-item>
      <flexbox-item>
      ...
      components:{
       ...
       data(){
        return{
          ...
          downCard:require('../assets/images/0-14png_03.png'),
        }
      },
      methods:{
       addFile(){
          ...
          this.getPhotoAndUpload(function(ret){
           ...
           localStorage.setItem('permit',ret.data.url)
            if (ret.code=200)
              vm.downCard = typeof ret.data.url!="undefined"? ret.data.url : vm.downCard;
          },function(r){
        ...
        updateData(){
          let vm=this;
            this.apiPost("http://r.huileyi.com/Merchant/sellerPerfectData",{type:"idcard",idcard_negative_img:vm.downCard}).then((res)=>{
            if(res.code=200){
                this.$vux.toast.show({
                  text: '上传成功',
                  type: "success"
