<template>
  <div class="main">
    <div class="content">
      ...
      <router-link v-if="stauts==1" to="/pretermit">
        <div class="cell">
      ...
      <div @click.stop='is_show=!is_show' class='topbar'>
        ...
        <div v-if='is_show' class='downPanel'>
           <div v-for='(i,index) in shoplist' v-show='type != index'  @click='onChange(i.store_id,i.store_name)'  class='downbar'>{{i.store_name}}</div>
        </div>
      </div>
      <div class="ipt">
        <div>
          <span class="lf clearfix">可提现金额</span>
          <span class="rt clearfix" style="color:#3879D9">￥{{money}}</span>
        </div>
        <div style="clear:both;"></div>
        <div>
    ...
    <div style="width:100%;background-color: #fff;">
      <div  style="font-size: 12px;text-align: center;padding:0 15px;">
        <div style="width:100%;">
      ...
      methods:{
        ...
        let regMoney=/^[1-9]\d*(,\d{3})*(\.\d{2})?$/;
          ...
          if(this.money.length==0){
            ...
            this.apiPost("http://r.huileyi.com/income/percentageCash", {card_id:vm.card_id,cash_money:vm.money,merchant_id:merchant_id}).then((res) => {
                 console.log(res)
                if(res.code==200){
                  this.$vux.toast.show({
                        text: res.msg,
