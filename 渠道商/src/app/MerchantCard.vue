      ...
      methods:{
        ...
        let reg_bink=/([\d]{4})([\d]{4})([\d]{4})([\d]{4})([\d]{0,})?/
          ...
          if(!reg_bink.test(this.bankcard)){
            ...
            let vm=this
            vm.apiPost(vm.rhost+'/home/doAboutBankCard',{account_type:'bankcard',type:'add',card_number:this.bankcard,card_bank:this.bank,owner_name:this.name,card_below:this.bankname}).then((res)=>{
              if(res.code==200){
                this.$vux.toast.show({
                    text: '绑定成功',
                    type: "success"
                })
                  this.$router.push({path: '/addMerchant?addMerchant=addMerchant'});
